...
@interface EngineView (PrivateMethods)
- (void) initGL;
...
{
    CVReturn result = [(EngineView*)displayLinkContext getFrameForTime:outputTime];
    return result;
}
...
- (void) reshape
{
    ...
    width = viewRectPixels.size.width;
...
- (void) drawView
{
	...
	//[m_renderer render];
    //TODO i dont think this is a good place for the update, do some research
    UpdateEngine();
...
-(void)mouseMoved:(NSEvent *)theEvent
{
    ...
    MouseMove(curPoint.x, curPoint.y);
}
...
-(void)rightMouseDragged:(NSEvent *)theEvent  // <-- lol?
{
    NSPoint curPoint = [self convertPoint:[theEvent locationInWindow] fromView:nil];
    MouseMove(curPoint.x, curPoint.y);
}
...
{
    NSPoint curPoint = [self convertPoint:[theEvent locationInWindow] fromView:nil];
    MouseDown(1, curPoint.x, curPoint.y);
...
{
    NSPoint curPoint = [self convertPoint:[theEvent locationInWindow] fromView:nil];
    MouseUp(1, curPoint.x, curPoint.y);
}

- (void)rightMouseDown:(NSEvent *)theEvent
{
    NSPoint curPoint = [self convertPoint:[theEvent locationInWindow] fromView:nil];
    MouseDown(2, curPoint.x, curPoint.y);
}

- (void)rightMouseUp:(NSEvent *)theEvent
{
    NSPoint curPoint = [self convertPoint:[theEvent locationInWindow] fromView:nil];
    MouseUp(2, curPoint.x, curPoint.y);
}
