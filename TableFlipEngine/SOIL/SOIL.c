...
//gl error checking and engine logging
...
//windows
	...
	#define APIENTRY
//android
#elif defined(__ANDROID__) || defined(__WEB_GL__)
        #include <GLES2/gl2.h>
...
//TODO compressed mobile textures? P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC soilGlCompressedTexImage2D = NULL;
...
//because i need width and height dangit!
unsigned int
        SOIL_load_OGL_texture_whc
        (
                const char *filename,
                int *_width, int *_height, int *_channels,
                int force_channels,
                unsigned int reuse_texture_ID,
                unsigned int flags
        )
	...
	)
{
    ...
    CheckGLErrors();
    printf("Max Supported Size: %d\n", texture_check_size_enum);
	...
	if( (width > max_supported_size) || (height > max_supported_size) )
	{
        ...
        printf("texture size is too large, reducing...\n");
	...
	if( tex_id )
	{
		...
		if( flags & SOIL_FLAG_TEXTURE_REPEATS )
		{
			...
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
                ...
                //NOTE webgl maynot like this, there is no wrap_R
		...
		} else
		{
			 ...
			 *below line WAS unsigned int clamp_mode = GL_CLAMP;*/
			...
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
                //NOTE webgl and ES may not have wrap_r
...
int query_DXT_capability( void )
{
	...
	if( has_DXT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		} else
		{
			...
			#elif defined(__ANDROID__) || defined(__WEB_GL__)
				//TODO add compress textures
				ext_addr = (P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC)NULL;
			#else
				ext_addr = (P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC)
